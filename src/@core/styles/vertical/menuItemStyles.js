...
const menuItemStyles = (verticalNavOptions, theme) => {
  ...
  return {
    ...
    icon: ({ level }) => ({
      ...
      ...(level > 0 && {
        fontSize: '1.375rem', // Changed from '0.75rem' to match level 0
        color: 'var(--mui-palette-text-primary)' // Changed from 'var(--mui-palette-text-secondary)' to match level 0
      }),
      ...(level === 0 && {
